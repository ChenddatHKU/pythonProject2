...
def floor(n):
  if float(n) < 0.001:
    return str(0.001)  # replace atof by float
  else:
    return n

infile = open('/Users/chendd/LowFreMut/screening/changed pos', 'r')
outfile = open('/Users/chendd/LowFreMut/screening/HCMut', 'w')
silfile = open('/Users/chendd/LowFreMut/screening/sil', 'w')
misfile = open('/Users/chendd/LowFreMut/screening/mis', 'w')
nonfile = open('/Users/chendd/LowFreMut/screening/non', 'w')
outfile2 = open('/Users/chendd/LowFreMut/screening/HCMu2(mis)', 'w')
header = "\t".join(['Pos', 'Mut', 'AA', 'HBXA-1-RF', 'HBXA-2-RF', 'LBXA-1-RF', 'LBXA-2-RF', 'passage-1-RF','passage-2-RF', 'HBXA-C-1-RF', 'HBXA-C-2-RF', 'LBXA-C-1-RF', 'LBXA-C-2-RF','passage-C-1-RF','passage-C-2-RF','Fit', "FitC", "DS-HBXA", "DS-LBXA", "DS-HBXA-C", "DS-LBXA-C"])
...
for line in infile.readlines():  # replace xreadlines by readlines
  ...
  if Pos > 24 and Pos < 745:
    DNA_RF = str(DNA / DNA_WT)   ## mut DAN baseline relative frequency to wt
    HBXA_1_RF = str(HBXA_1 / HBXA_1_WT)
    HBXA_2_RF = str(HBXA_2 / HBXA_2_WT)

    LBXA_1_RF = str(LBXA_1 / LBXA_1_WT)
    LBXA_2_RF = str(LBXA_2 / LBXA_2_WT)

    passage_1_RF = str(passage_1 / passage_1_WT)
    passage_2_RF = str(passage_2 / passage_2_WT)


    HBXA_C_1_RF = str(HBXA_C_1 / HBXA_C_1_WT)
    HBXA_C_2_RF = str(HBXA_C_2 / HBXA_C_2_WT)

    LBXA_C_1_RF = str(LBXA_C_1 / LBXA_C_1_WT)
    LBXA_C_2_RF = str(LBXA_C_2 / LBXA_C_2_WT)

    passage_C_1_RF = str(passage_C_1 / passage_C_1_WT)
    passage_C_2_RF = str(passage_C_2 / passage_C_2_WT)

    # passage_RF = str( (passage_1 + passage_2) / (passage_1_WT + passage_2_WT) )  ###remember there is an issue here, which type of average is better?
    # passage_C_RF = str( (passage_C_1 + passage_C_2) / (passage_C_1_WT + passage_C_2_WT) )
...
# differential selection calculation
    ...
    # dRF_HBXA_1 = str(float(HBXA_1_RF) / float(passage_RF))
    # dRF_HBXA_2 = str(float(HBXA_2_RF) / float(passage_RF))
    # dRF_LBXA_1 = str(float(LBXA_1_RF) / float(passage_RF))
    # dRF_LBXA_2 = str(float(LBXA_2_RF) / float(passage_RF))
    #
    # dRF_HBXA_C_1 = str(float(HBXA_C_1_RF) / float(passage_C_RF))
    # dRF_HBXA_C_2 = str(float(HBXA_C_2_RF) / float(passage_C_RF))
    # dRF_LBXA_C_1 = str(float(LBXA_C_1_RF) / float(passage_C_RF))
    # dRF_LBXA_C_2 = str(float(LBXA_C_2_RF) / float(passage_C_RF))
    ...
    out = "\t".join([str(Pos), G, F1, HBXA_1_RF, HBXA_2_RF, LBXA_1_RF, LBXA_2_RF, passage_1_RF, passage_2_RF, HBXA_C_1_RF, HBXA_C_2_RF, LBXA_C_1_RF, LBXA_C_2_RF, passage_C_1_RF, passage_C_2_RF,fit, fitC, dRF_HBXA, dRF_LBXA, dRF_HBXA_C, dRF_LBXA_C])
    outfile.write(out + "\n")
